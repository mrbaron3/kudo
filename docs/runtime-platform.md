...
IssueとReviewに同じproviderを指定してもよいが、session、credential、filesystem、contextはroleごとに分離する。選択したprovider/model/adapter version/tool/timeout policyはRun開始時にExecution Policy artifactとして固定する。
